...
public class Screen extends Canvas implements Runnable
{
    ...
    public void tick()
    {
        ...
        for(int i = 0; i < objectCounter; i++)
        {
            Block object = (Block) coreObjects[i];
            object.tick();
        }
        ball.tick();
    ...
    public void reset()
    {
        coreObjects = new CoreObject[25];
        objectCounter = 0;

        ball = new Ball(Reference.CENTER_X - 5, Reference.CENTER_Y, 10, 10, Color.CYAN );
        paddle = new Paddle(Reference.CENTER_X - 25,Reference.HEIGHT -30,50,15, Color.cyan);
        leftWall = new Wall(0, 0, -30,Reference.HEIGHT, Color.red);
        rightWall = new Wall(Reference.WIDTH, 0, 30, Reference.HEIGHT,Color.black);
        ball.setVelY(-2);
        ball.setVelX(-2);
        int X = 20;
        int Y = 20;
        for(int i = 0; i < 13; i++)
        {
            ...
            coreObjects[i] = new Block(X, Y, 40, 20, Color.ORANGE);
            objectCounter++;
            X += 45;
        }
    }
